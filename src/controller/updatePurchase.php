...
$produitID = sanitizeInput($_POST['Show-purchaseID']);
...
$query = "UPDATE `purchase` SET `quantity`= ? ,`price`= ?,`update_date`= CURRENT_TIMESTAMP() ,`Note`= ? WHERE `id`= ? ";
...
if ($res > 0) {
    ...
    echo "Success: Purchase updated successfully.";
} else {
