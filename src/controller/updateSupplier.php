...
if ($imageRemoved) {
    ...
    $image = getSingleValue($conn, 'suppliers', 'image', "id = '$id'");

    if ($image != "user-default.png") {
        $str = removeImage($image, $uploadDir);
...
SET `business_name`=?, `prenom`=?,`nom`=?,`address`=?,`adress2`=?,`code_postale`=?,`Ville`=?,`Pays`=?,`phone`=?,`email`=?,`gender`=?,`updated_at`= CURRENT_TIMESTAMP() $part WHERE `id`= ?";
