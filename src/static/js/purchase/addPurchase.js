$(document).ready(function () {
  ...
  $("#add-purchase-button").click(function () {
    ...
    let inputs = ["produitID", "prixActuel", "stockActuel", "note"];
    ...
    $.ajax({
      url: "../controller/addPurchase.php",
      method: "POST",
      data: formData,
      processData: false,
      contentType: false,
      success: function (response) {
        ...
        console.log(response);
        if (response == "Success: Purchase Affectted successfully.") {
          resetForm(formData);
          $("#myModal").modal("hide");
          table.ajax.reload();
        } else {
...
function resetForm(formData) {
  ...
  $("#add-purchase-button").html("Add Supplier");
  $("#add-purchase-button").removeClass("disabled");
